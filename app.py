...
# Semantic similarity – use sentence-transformers if available, else fallback to TF-IDF
...
try:
    ...
    try:
        ...
        SEMANTIC_AVAILABLE = False  # we'll use TF-IDF fallback
    except ImportError:
        SEMANTIC_AVAILABLE = None   # no semantic capabilities

# -------------------- CONFIGURATION (now environment‑aware) --------------------
class ComplianceConfig:
    """Central, auditable configuration with environment overrides."""
    # IRS limits
    ...
    OPENSTATES_JURISDICTION = "Ohio"   # hardcoded for pilot – can be made dynamic
    API_TIMEOUT = 15
...
def fetch_bill_metadata(query: str, api_key: str) -> Dict:
    ...
    try:
        resp = requests.get(
            ComplianceConfig.OPENSTATES_BASE_URL,
            headers=headers,
            params=params,
            timeout=ComplianceConfig.API_TIMEOUT
        )
        ...
        # Extract the most recent version URL
        text_url = None
...
def compute_semantic_differences(policy_text: str, bill_text: str) -> Dict:
    """
    Advanced NLP comparison using sentence embeddings.
    Returns a dict with similarity score, risky sentences, and alignment.
    """
    if not policy_text or not bill_text:
        return {"error": "Missing text for comparison."}
    
    # Truncate to avoid OOM on large documents
    ...
    if SEMANTIC_AVAILABLE and ComplianceConfig.ENABLE_SEMANTIC:
        try:
            ...
            # For each policy sentence, find most similar bill sentence and score
            similarities = []
            ...
            return {
                ...
                "sentence_analysis": similarities[:20]  # limit for UI
            }
        except Exception as e:
...
# -------------------- TRADITIONAL DIFF (Enhanced) --------------------
def render_side_by_side_diff(policy_sentences, bill_sentences):
    """Improved diff visualization with context."""
...
# -------------------- LEGISLATIVE REDLINER MODULE (Enterprise) --------------------
def render_legislative_redliner():
    ...
    if not api_key:
        st.error("🚨 **Open States API key not found.** Please add it to `.streamlit/secrets.toml`")
        st.stop()
    ...
    with st.container():
        ...
        with col1:
            bill_query = st.text_input(
                "🔎 **Search Ohio Bill** (e.g., HB33, SB1, 'minimum wage')",
                value="HB33",
                key="bill_query_input"
            )
        with col2:
    ...
    if st.session_state.bill_data and "error" not in st.session_state.bill_data:
        ...
        cols[2].metric("Status", bill.get('status', 'N/A').capitalize())
        ...
        # Option to load sample or paste
        ...
        if st.button("🔬 Run Compliance Gap Analysis", disabled=analyze_disabled, type="primary", use_container_width=True):
            with st.spinner("🧠 Analyzing semantic alignment..."):
                ...
                # Traditional diff
                diff_result = {"policy": policy_sentences, "bill": bill_sentences}
...
# -------------------- UPGRADED EXISTING MODULES --------------------
# (I've kept your original logic but added caching, better UI, and error handling.
#  For brevity, I'm showing the Tuition module with enhancements; the others follow the same pattern.)
...
def render_redliner_module():
    # ... (your original redliner, but with the improved diff view and semantic option if desired)
    # I'm keeping it concise here; you can replace with the same side‑by‑side diff used above.
...
def render_projector_module():
    # ... (your original timeline projector – already solid)
...
def main():
    ...
    <style>
        ...
        .main-header { font-size: 2rem; font-weight: 600; color: #0b3b5c; }
